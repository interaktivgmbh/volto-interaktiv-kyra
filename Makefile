...
PROJECT_NAME=clean-volto-template
STACK_NAME=clean-volto-template-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://clean-volto-template.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t collective/clean-volto-template-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t collective/clean-volto-template-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name clean-volto-template-frontend-acceptance --link clean-volto-template-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d collective/clean-volto-template-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name clean-volto-template-backend-acceptance -d collective/clean-volto-template-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop clean-volto-template-frontend-acceptance
	@docker stop clean-volto-template-backend-acceptance
