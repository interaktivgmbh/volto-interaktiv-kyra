...
DEPLOY_ENV=prod
DEPLOY_HOST=clean-volto-template.example.com
DEPLOY_PORT=22
DEPLOY_USER=plone
...
You need either a Ubuntu or Debian based system for clean-volto-template.example.com, enable SSH, and install a supported version of Python 3 on that system.
...
prod:
  hosts:
    clean-volto-template.example.com:
      ansible_user: root
      host: clean-volto-template
      hostname: clean-volto-template.example.com
```
...
Deploy the stack defined in `devops/stacks/clean-volto-template.example.com.yml` to the remote server with:
